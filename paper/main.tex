...
part alone contributes $E$.  For sharpness, let $u_n=n^{-1/2}(1,\ldots,1)$
and set $G_n=I+t_nu_nu_n^*$, where
$t_n=\sqrt{E/(1-1/n)}$.  Then the off-diagonal energy of $G_n$ is exactly
$E$, while $\tr\Psi(G_n)=\Psi(1+t_n)$ tends to $2\sqrt E-1$ for $E\ge1$
and to $E$ for $0\le E\le1$.  Thus no larger lower profile is possible.
\end{proof}
...
\end{equation}
Put $L=A/B_p=47603/100$.  If $\mathrm{span}(B)\ge L$, this is immediate:
$\tr\Psi(G_B)\ge0$ and
$\eta B_p\,\mathrm{span}(B)\ge\eta A=R$.  We may therefore suppose that
$\mathrm{span}(B)<L$.  Every pair separation in $B$ then belongs to the
fixed compact interval $[0,L]$, so the compact-uniform Gram-entry asymptotic
of Input~\ref{inp:stability} applies uniformly over these blocks.  It gives
$2\sum_{i<j}w(y_j-y_i)\le E_B+o(1)$; together with
\eqref{eq:blockenergy},
\[
...
independent runs of the identical decision procedure: the headline target
