...
\end{equation}
The final equality is the Riemann--von Mangoldt formula.
\end{impinput}
...
\[
 L:=A/B_p=714127/1500.
\]
If $\mathrm{span}(B)\ge L$, \eqref{eq:blockdefect} is immediate:
$\tr\Psi(G_B)\ge0$ and
$\eta B_p\,\mathrm{span}(B)\ge\eta A=R$.  We may therefore suppose that
$\mathrm{span}(B)<L$.  Every pair separation in $B$ then lies in the fixed
compact interval $[0,L]$.  The Gram-entry asymptotic of
Input~\ref{inp:stability} is uniform there, and hence
\[
 2\sum_{i<j}w(y_j-y_i)\le E_B+o(1).
\]
...
Because $m$ and $L$ are fixed, the $o(1)$ here is uniform over the
...
size $m$.  Pinching does not increase $\tr\Psi$ because
$X\mapsto\tr\Psi(X)$ is convex and unitarily invariant; hence
$\tr\Psi(M)$ dominates the sum of the block defects.  Since there are
$O(N)$ blocks, the uniform per-block $o(1)$ above sums to $o(N)$.  Summing
\eqref{eq:blockdefect} over the full blocks of one offset and averaging over
the $m$ offsets therefore gives, with the retained ordinates of
Input~\ref{inp:span}, each interior gap
